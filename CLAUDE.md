...
- **모델 A 팀(docs·design·pm 등)은 에이전트만 복사해 와도 된다** — `/team-assemble`가 동반 자산(스킬·`templates/`·
  참조 문서 `docs/`·`config/`·`.mcp.json` 병합)을 **HQ 레포에서 자동 포트**한다(같은 머신·경로 확보 시, SKILL §2-1b).
  환경변수 설정·브라우저 OAuth 1회 인증만 수동이다. HQ 레포가 없는 환경이면 4종+참조 문서를 수동 이식한다(SKILL §1-1 표).
