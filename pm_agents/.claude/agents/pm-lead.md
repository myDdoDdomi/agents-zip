...
> 에이전트에 전달(각자 재조회 금지). 같은 JQL 중복 호출 금지. 확정 후 실행. 대량 JQL은 `fields`·
> `maxResults` 제한 + `startAt` 페이지네이션으로 분할(응답 토큰 한도 초과 시 파일 저장 후 파싱 —
> `docs/ATLASSIAN-MCP.md` §3-2). (→ CLAUDE.md §3-1)
...
5. **외부 발신 승인·집행**: Jira 쓰기가 필요하면 변경 계획 표를, Slack 게시가 필요하면 게시 미리보기(대상
   ...
   변경 계획 표가 누락되면 `pm-reviewer`가 즉시 🔴 차단한다(실사례 2026-06-11) — 표를 먼저 완성해 제출한다.
