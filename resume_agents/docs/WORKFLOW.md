...
- 이력서 `/resume-build`(`resume-writer`), 포트폴리오 `/portfolio`(`portfolio-writer`).
- **평문 `.md` 파일로 먼저 산출(표준).** 작성 결과는 검수·Drive 반영 전에 **반드시 로컬 평문 `.md`
  파일**(예: `[회사]_[직무]_이력서.md`)로 먼저 만든다. 이후 단계(검수·import)는 **이 `.md`를 단일
  원본(single source of truth)으로** 사용한다. (이 방식이 가장 매끄럽고, Drive 양식 변환 마찰을 피한다.)
...
## 플랫폼 제약 (참고)
- **에이전트 이어받기 불가:** 한 번 띄운 작성/포맷 에이전트를 그대로 이어 부르는 기능(SendMessage 등)은
  현재 플랫폼 제약으로 지원되지 않습니다. 단계 간 컨텍스트는 ③의 **평문 `.md` 산출물**과 ④의 **검수 판정
  블록**(구조화)으로 넘겨, 재전달 부담과 토큰 낭비를 줄입니다.
