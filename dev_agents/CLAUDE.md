...
- [ ] (외부/백엔드 계약 의존 시) 콜러블 계약 정합 + rules↔쿼리 정합 + 복합 인덱스 점검 완료 — 계약 드리프트는 **4차원(casing·필수 필드·부호/방향·상태 시점)**으로 대조하고, 상태 스냅샷 전달은 시점(변경 전/후) 주석 명시 + 대응 테스트에서 계약 값 실값 검증(`expect.any` 류 금지)
- [ ] (멀티서비스 파이프라인 변경 시) 실계약 fixture 또는 실 통합 호출로 end-to-end 흐름 검증 — mock 단독 검증은 완료 불가
- [ ] (코어 데이터 생성 경로에 신규 try/except 추가 시) 실패 로그 레벨·관측성·침묵 UI 영향을 `code-reviewer`가 확인(silent catch가 코어 출력을 무음 비동작시키지 않게)
- [ ] (mock/스텁이 있는 코드베이스) mock-prod 패리티 확인 — 스텁(`return []`/`null`/not-implemented) 미구현이 실환경에 남지 않음
- [ ] (백엔드/서버리스 핵심 경로 변경 시) 로컬 통합 환경(에뮬레이터 등) 스모크 통과
- [ ] (프로덕션 배포 시) **배포 전 git 커밋** — 미커밋 working tree 배포 금지(롤백·이력 추적 보장)
- [ ] (FE-BE 통합 완료 시점) QA 핸드오프(체계적 감사)를 **배포 전·사용자 보고 전에 dev 팀 주도로** 실행 — **UI 아키텍처 변경(컴포넌트 교체·상호작용 패턴 전환) 포함 시 핸드오프 없는 프로덕션 배포 불가**(자체 단위테스트 green은 충분 조건 아님)
- [ ] `code-reviewer` 리뷰 통과(품질·버그·코드레벨 보안)
- [ ] 비밀값 하드코딩·시크릿 노출 없음 — 비밀값은 채팅으로 받지 않고 env/시크릿 설정 방법으로 안내
