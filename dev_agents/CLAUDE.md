...
- [ ] (외부/백엔드 계약 의존 시) 콜러블 계약 정합 + rules↔쿼리 정합 + 복합 인덱스 점검 완료 — 모델 dump를 다른 서비스에 직접 전달하면 상대 required 필드(존재 여부)까지 대조(casing 넘어)
...
- [ ] (FE-BE 통합 완료 시점) QA 핸드오프(체계적 감사)를 **배포 전·사용자 보고 전에 dev 팀 주도로** 실행
