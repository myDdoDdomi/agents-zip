...
> 디자인 팀과 개발 팀을 **한 프로젝트에서 함께** 쓰려면 본부의 `/team-assemble`로 두 팀을 조립하세요. 디자인 팀(모델 A)은 에이전트뿐 아니라 스킬·템플릿·Figma 설정까지 함께 옮겨야 하고, `dev_agents`(복사해서 쓰는 팀)는 `agents/*.md`만 복사합니다. → 자세한 복사 목록·순서는 바로 아래 **[다른 팀과 함께 쓰기]** 섹션을 보세요.
