...
> 디자인 팀과 개발 팀을 **한 프로젝트에서 함께** 쓰려면 본부의 `/team-assemble`로 두 팀을 조립하세요(에이전트를 `.claude/agents/<팀>/`로 복사 → 자동 정리). `dev_agents`는 **복사해서 쓰는 팀**이라, 그 팀의 `agents/*.md`를 대상 프로젝트 `.claude/agents/`로 복사합니다. 자세히는 루트 `README.md`/`CLAUDE.md`.
