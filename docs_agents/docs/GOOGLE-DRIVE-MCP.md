# Google Drive(Docs) MCP 연결 가이드

에이전트가 Google Drive 문서를 직접 읽고 쓰려면 **Google Workspace MCP 서버**를
연결해야 합니다. 팀 공유 설정은 레포 루트 `.mcp.json`에 이미 들어 있고,
...
> — Drive/Docs 읽기·쓰기를 지원합니다. (다른 Drive MCP를 쓰고 싶으면 §대안 참고)
...
5. 발급된 **클라이언트 ID / 시크릿**을 안전하게 보관 (레포에 커밋 금지)

### 3) 환경변수 설정
`.mcp.json`은 `${VAR}` 형태로 환경변수를 읽습니다. 아래를 본인 PC에 설정하세요.

**Windows (PowerShell, 영구 설정):**
```powershell
setx GOOGLE_OAUTH_CLIENT_ID    "발급받은-클라이언트-ID"
setx GOOGLE_OAUTH_CLIENT_SECRET "발급받은-시크릿"
setx USER_GOOGLE_EMAIL         "본인@gmail.com"
setx GOOGLE_MCP_CREDENTIALS_DIR "%USERPROFILE%\.google-mcp"
```
(설정 후 터미널/Claude Code 재시작)

**macOS/Linux (`~/.zshrc` 또는 `~/.bashrc`):**
```bash
export GOOGLE_OAUTH_CLIENT_ID="발급받은-클라이언트-ID"
export GOOGLE_OAUTH_CLIENT_SECRET="발급받은-시크릿"
export USER_GOOGLE_EMAIL="본인@gmail.com"
export GOOGLE_MCP_CREDENTIALS_DIR="$HOME/.google-mcp"
```
...
3. 첫 Drive 작업 시 브라우저로 **Google 로그인/동의** 창이 뜹니다 → 승인.
   이후 토큰이 `GOOGLE_MCP_CREDENTIALS_DIR`에 저장돼 재인증이 필요 없습니다.
...
| 토큰 만료 반복 | 자격 디렉터리 권한 | `GOOGLE_MCP_CREDENTIALS_DIR` 경로/권한 확인 |
