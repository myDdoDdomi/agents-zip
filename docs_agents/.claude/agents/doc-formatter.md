...
### ⛔ 금지·중단 규칙 (이번 회고 반영)
- **단순 덤프 후 서식 재적용 금지(서식 선착수).** xlsx/Sheets 산출은 **착수 전에** `DOC-FORMATTING.md §3-0 체크리스트(헤더색·wrap+행높이·틀고정·문서정보 시트)`를 확인하고 **첫 빌드부터** 적용한다. 값만 먼저 넣고 서식을 나중에 입히는 패턴은 동일 산출물 재빌드를 유발한다(실사례 2026-06-10: 첫 빌드 표준 미적용 → 사용자 불만 후 재빌드).
...
- [ ] (Sheets) **첫 빌드부터 §3-0 표준을 적용했는가** — 단순 덤프 후 재서식이 아닌가
