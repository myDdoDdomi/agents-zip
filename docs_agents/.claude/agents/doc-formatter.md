...
## 작업 절차 — Google Sheets
> 작성/서식의 절대 기준은 `docs/DOC-FORMATTING.md` **§3(형식) + §3-1(작성법·도구·레이아웃·안전절차)** 이다. 아래는 요약.
1. **생성/열기**: 새로 만들면 `create_spreadsheet(title)`로 빈 시트 생성 후 `spreadsheet_id` 확보. 기존이면 ID로 열고 먼저 현재 값을 읽는다.
2. **시트 구성**: 본문 시트 + `문서정보` 시트. **헤더는 1행, 데이터는 2행부터** 쓴다(range는 `시트명!A2:…`).
3. **컬럼 = 템플릿 일치**: 헤더 컬럼은 §3-1-C-1의 문서종류별 레이아웃(=`templates/`의 표 컬럼)을 따른다. 각 행=한 항목.
4. **값 쓰기**: `modify_sheet_values(spreadsheet_id, range, values=2D배열)`로 한 번에. (도구명은 호출 전 `/mcp`로 확인)
5. **서식**: 헤더 네이비 배경(`#1F3A5F`)+흰 굵은 글씨+가운데, 헤더 1행 고정, 자동필터, 옅은 테두리(`#D0D5DD`), 상태/우선순위 색 구분.
   - ⚠️ 이 서버의 **셀 서식 전용 도구 시그니처가 불확실**하면 지어내지 말 것 → §3-1-E 폴백: 값은 채우고 미적용 서식을 사용자에게 명시(`(확인 필요)`).
6. **수정 안전절차(§3-1-D)**: ① 먼저 읽고 ② 덮어쓸 range를 정하고 ③ 기존 행 **수정 vs 끝에 추가**를 구분(밀림 주의) ④ **헤더 1행 보존**.
7. 상단 또는 `문서정보` 시트에 메타정보(버전/작성자/일자) 블록.
8. **⭐ 가독성 마무리 (필수 — 마지막 단계, §3-0)**: 값을 다 채운 뒤 반드시 **셀 내용이 잘리지 않게** 마무리한다.
   **열 너비 자동맞춤**(헤더보다 좁지 않게, 긴 열은 상한 폭) + **긴 텍스트 열 줄바꿈(wrap) ON + 행 높이 자동확장(반드시 세트)** + **세로 상단 정렬·가로 텍스트 왼쪽/숫자상태 가운데**. wrap만 켜고 행높이가 안 늘면 여전히 잘리므로 둘을 함께. 도구가 불확실/부재면 §3-1-E 폴백으로 사용자에게 수동 적용 안내.
...
- [ ] 표 헤더가 네이비 배경 + 흰 글씨인가
- [ ] 핵심 결론이 강조 박스로 보이는가
- [ ] (Sheets) 헤더 1행 고정·네이비색·필터가 적용됐고, 컬럼이 템플릿과 일치하며, 수정 시 헤더 1행을 보존했는가
- [ ] (Sheets) ⭐ **셀 내용이 잘리지 않는가** — 열 너비 자동맞춤 + 긴 열 줄바꿈+행 높이 자동확장(세트) + 상단 정렬(§3-0)
- [ ] 새 문서를 지정 폴더로 이동했는가
